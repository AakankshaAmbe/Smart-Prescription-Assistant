// ============ RESULTS.JS - COMPLETE FIXED VERSION ============
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    sessionStorage.getItem("previewData"); // ✅ ADD THIS
    ...
    data.medicines.forEach((m, index) => {
        ...
        card.innerHTML = `
            ...
            <p><strong>⚠️ Side Effects:</strong> ${m.side_effects?.replace(/⚠️ Side Effects:/g, "").trim() || "Not available"}</p>
...
async function generateVoiceInstructions() {
    ...
    try {
        const response = await fetch(`http://localhost:8000/api/voice?lang=${language}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                patient_name: data.patient_name || "Patient",
...
async function downloadPrescriptionCard() {
    ...
    try {
        const userId = localStorage.getItem('userId');
        const response = await fetch(`http://localhost:8000/api/card?lang=${language}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
